...
class GenericWave():
    ...
    def factor_check(self, num=5):
        """
        Check if total number of point is factorable by num. If not, add 0s to
        tail to satisfy this requirement.

        Parameters
        ----------
        num : int, optional
            Factor. The default is 5.
        ...
        if points == 0:
            return
        points = num - points
        add_length = self.dx * points
        self.set_total_length(self.get_span(False) + add_length)
...
class Wave(GenericWave):
    ...
    def __init__(self, funcHandle=None, variables=[], properties={}):
        ...
        self.dx = copy(self.x[1] - self.x[0])
    ...
    def get_appendRule(self, show=True):
        """
...
class Waveform(GenericWave):
    ...
    def set_total_length(self, total_length):
        """
        Set method for waveform's total length.

        Parameters
        ----------
        total_length : float
            Indicted total length (same unit as x) and must larger than object
            span.
        ...
        """
        if self.get_span(False) >= total_length:
            print("total_length too short!")
            return
        points = int(total_length / self.dx + 1)
        nullWave = Waveform._nullBlock(total_length, points)
        reference = Waveform(nullWave, nullWave[0].get_name(False))
        self <<= reference
    ...
    def offset(self, offset=0, add_tail=False):
        ...
        """
        if offset == 0 or offset < 1e-9:
            return self
        points = int(offset / self.dx + 1 )
        new_waveList = (
            self.get_waveList() + Waveform._nullBlock(
                offset, points) if add_tail else
            Waveform._nullBlock(
                offset, points) + self.get_waveList()
            )
        return Waveform(new_waveList)

    @classmethod
    def _nullBlock(cls, span=.0, points=0, appendRule=[True, True]):
...
class QubitChannel(GenericWave):
    ...
    def get_wire_names(self, show=True):
        """
    ...
    def set_total_length(self, total_length):
        ...
        if self.get_span(False) >= total_length:
            ...
            return
        points = int(total_length / self.dx + 1)
        reference = Waveform(Waveform._nullBlock(total_length, points))
        newList = []
        for waveform in self.wires:
            waveform <<= reference
            newList += [waveform]
