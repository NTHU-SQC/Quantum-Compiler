...
class GenericWave():
    ...
    def psd(self, dBm_scale=True):
        ...
        df = 1 / self.dx
        f, Pxx_den = welch(
            self._y, fs=df, window=np.ones(nfft),
...
class QubitChannel(GenericWave):
    ...
    def __mod__(self, wireList):
        """
        Shorthand method for del_wire() method. Denoted as self / qcObj.

        Parameters
        ----------
        wireList : list
            List of wire name strings or indices to be removed from
            QubitChannel object.
    ...
    def del_wire(self, wireList=[]):
        """
        Remove wires from QubitChannel object.

        Parameters
        ----------
        wireList : list
            List of QubitChannel wire name strings or indices.
        ...
        """
        idxList = [
            self.wire_names.index(item) if isinstance(item, str)
            else item for item in wireList
            ]
        wires = [
            self._wires[idx] for idx in range(len(self._wires))
